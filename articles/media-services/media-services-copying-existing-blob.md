<properties 
    pageTitle="Olemassa olevan Blob-objektien kopioiminen siihen Media Services resurssi | Microsoft Azure" 
    description="Tässä ohjeaiheessa esitellään, miten kopioida aiemmin Blob-objektien Media-palveluiden resurssi." 
...
#<a name="copying-an-existing-blob-into-a-media-services-asset"></a>Olemassa olevan Blob-objektien kopioiminen siihen Media-palveluiden resurssi

Tässä ohjeaiheessa esitellään, miten kopioida BLOB storage tililtä uusi Microsoft Azure Media Services-resurssi.

Oman BLOB voi olla tallennustilan-tili, joka on liitetty Media Services-tilin tai tallennustilan tilin, joka ei ole liitetty Media Services-tiliin. Tässä ohjeaiheessa kerrotaan, miten kopioida BLOB storage tililtä Media-palveluiden resurssi. Huomaa, että voit myös kopioida tietoja keskikohdan mukaan yli. Kuitenkaan voi olla näin aiheutuvat kulut. Saat lisätietoja hinnat [Tiedonsiirron](https://azure.microsoft.com/pricing/#header-11).
...
Hae ja suorita otoksen [täältä](https://azure.microsoft.com/documentation/samples/media-services-dotnet-copy-blob-into-asset/).

##<a name="prerequisites"></a>Edellytykset

- Uusi tai aiemmin luotu Azure tilauksen kaksi Media Services tilit. Aiheessa [Media Services-tilin luominen](media-services-portal-create-account.md).
...
1. Visual Studio avulla voit luoda uuden ratkaisun, joka sisältää C#-konsolisovelluksen projektin. 
2. Kirjoita CopyExistingBlobsIntoAsset nimi ja valitse sitten OK.
...
1. Lisää muita viittauksia, joita tarvitaan projektin: System.Configuration.
...
##<a name="copy-blobs-from-a-storage-account-into-a-media-services-asset"></a>Kopioi BLOB Storage tililtä Media Services-kohteiden tuominen

Alla olevassa esimerkissä koodi suorittaa seuraavia tehtäviä:

1. Luo CloudMediaContext esiintymä. 
1. Luo CloudStorageAccount ilmentymät: _sourceStorageAccount ja _destinationStorageAccount.
1. Lataa tasainen Streaming-tiedostoja paikallisen hakemistosta blob säilöön, joka sijaitsee _sourceStorageAccount. 
1. Luo uusi resurssi. Blob-säilö, joka on luotu kohteen sijaitsee _destinationStorageAccount. 
1. Kopioi määritetyn BLOB liittyvän kohteen säilöön käyttämällä Azure-tallennustilan SDK.

    >[AZURE.NOTE]Kopiointi palauttaa poikkeuksen ei, jos locator on vanhentunut.

1. Koska tässä esimerkissä on tasainen streaming tiedostoja kopioidaan esimerkissä määrittämisestä .ism tiedoston ensisijainen tiedosto. Jos esimerkiksi on kopioitu .mp4-tiedoston, mp4-tiedoston määrittää ensisijainen-tiedosto.
1. Luo kohteen liittyvän OnDemandOrigin locator tasainen Streaming URL-osoite. 
