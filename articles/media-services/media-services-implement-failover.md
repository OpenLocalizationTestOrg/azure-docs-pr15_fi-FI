<properties 
    pageTitle="Käyttöönoton streaming skenaarion automaattisesti | Microsoft Azure" 
    description="Tässä ohjeaiheessa esitellään ottamisesta käyttöön streaming skenaarion automaattisesti." 
...
#<a name="implementing-failover-streaming-scenario"></a>Käyttöönoton streaming skenaarion automaattisesti

Tätä vaiheittaista esittelee kopioiminen sisällön (BLOB) yksi resurssi toiseen jotta voit käsitellä redundancy varten tarvittaessa streaming. Tässä skenaariossa on hyötyä asiakkaille, jotka haluat määrittää niiden CDN voit siirtää kaksi palvelinkeskusten kyseessä johonkin Microsoftin tietojen keskikohdan mukaan käyttökatkosta välillä.
Tätä vaiheittaista käytetään Microsoft Azure Media Services SDK, Microsoft Azure Media Services REST API ja Azure-tallennustilan SDK osoittamaan seuraavat toimet.

1. "Tietokeskuksen a" Media Services-tilin määrittäminen.
1. Mezzanine-tiedoston lataaminen lähde-kohteiden tuominen.
1. Koodata kohteen usean bit korko MP4-tiedostoiksi. 
1. Luo vain luku-Suojaussidosten locator, on lukuoikeudet säilöön tallennustilan tili, joka on liitetty lähde-kohteen lähde-annetaan.
1. Pyydä vain luku-Suojaussidosten locator, edellisessä vaiheessa luotu säilössä lähde-kohteen. Nämä tiedot annettava kopioimiseen BLOB storage välillä (kuvaus artikkelin.)
1. Luo origin-locator, joka on luotu koodauksen tehtävän annetaan. 

Valitse käsittelemään vikasietotilaa:

1. "Data Centerissä b" Media Services-tilin määrittäminen.
1. Luo kohde tyhjä resurssi kohde Media Services-tilin.
1. Luo kirjoitus-Suojaussidosten locator, kohde tyhjä annetaan on kirjoitusoikeus säilöön kohde-tallennustilan tilin, joka on liitetty kohde kohteen.
1. Azure-tallennustilan SDK avulla voit kopioida BLOB-objektit (resurssi-tiedostoja) lähde-tallennustilan tilin "Tietokeskuksen a" ja "Data Centerissä B" kohde-tallennustilan tilin välillä (tallennustilan näissä tileissä on liitetty varat halutut.)
1. Liitä BLOB (resurssi-tiedostoja), joka on kopioitu kohde-kohteen kanssa kohde blob-säilö. 
1. Luo origin-locator, "Data Centerissä B" annetaan ja määritä locator tunnus, joka on luotu, annetaan "Tietokeskuksen a". 
1. Näin streaming URL-osoitteet, jossa URL-osoitteet suhteelliset polut ovat samat (vain kantaluku URL-osoitteet ovat eri). 
 
Tämän jälkeen käsittelemään minkä tahansa katkokset voit luoda CDN nämä origin locators päälle. 

Ottaa huomioon seuraavat asiat:

- Media Services SDK nykyinen versio ei tue luomisesta locator määritetyn locator tunnuksella. Käytämme Media Services REST API saavuttamiseksi tämän tehtävän.
- Media Services SDK nykyinen versio ei tue ohjelmallisesti luotaessa IAssetFile tietoja, jotka liittää sijoituksen resurssi-tiedostoja. Käytämme CreateFileInfos Media Services REST-Ohjelmointirajapinnalla saavuttamiseksi tämän tehtävän. 
- Tallennustilan salattu resurssit (AssetCreationOptions.StorageEncrypted) ei tueta replikoinnin (koska salausavaimen on erilainen molemmat Media Services-tilit). 
- Jos haluat hyödyntää Dynaaminen pakkaus, sinun täytyy ensin saada vähintään yksi tarvittaessa Streaming varatut resurssit. Lisätietoja on artikkelissa [Dynaamisesti pakkaus resurssit](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Kannattaa käyttää Media Services [Monistus työkalun](http://replicator.codeplex.com/) vaihtoehtona soveltamisesta automaattisesti streaming tilanne manuaalisesti. Tämän työkalun avulla voit toistaa varat kaksi Media Services-tilien välillä.

##<a name="prerequisites"></a>Edellytykset
 
- Uusi tai aiemmin luotu Azure tilauksen kaksi Media Services tilit. Katso, [miten voit luoda Media Services-tilin](media-services-portal-create-account.md).
- Käyttöjärjestelmät: Windows 7: ssä, Windows 2008 R2 tai Windows 8: ssa.
- .NET framework 4.5 tai .NET Framework 4.
- Visual Studio 2010 SP1 tai uudempi versio (Professional, Premium, Ultimate tai Express).
 
##<a name="set-up-your-project"></a>Projektin määrittäminen

Tässä osassa luodaan ja C#-konsolisovelluksen projektin määrittäminen.

1. Visual Studio avulla voit luoda uuden ratkaisun, joka sisältää C#-konsolisovelluksen projektin. Kirjoita HandleRedundancyForOnDemandStreaming nimi ja valitse sitten OK.
1. Luo SupportFiles-kansion HandleRedundancyForOnDemandStreaming.csproj projektitiedoston samalla tasolla. Luo OutputFiles ja MP4Files kansiot SupportFiles-kansioon. Kopioi .mp4-tiedosto (Tässä esimerkissä käytetään BigBuckBunny.mp4-tiedostoa) MP4Files-kansioon. 
1. **Nuget** avulla voit lisätä Media Services liittyvät dll viittauksia. Visual Studio Päävalikko, valitse Työkalut -> kirjasto pakettien hallinta -> paketti hallinta-konsolin. Konsolin ikkunan Kirjoita Asenna paketti windowsazure.mediaservices ja paina enter.
1. Lisää muita viittauksia, joita tarvitaan projektin: System.Configuration dataContractSerializer ja System.Web.
1. Korvaa käyttäen lausekkeita, jotka on lisätty Programs.cs tiedoston oletusarvoisesti seuraavat niistä:
...
1. AppSettings-osan lisääminen .config-tiedostoon ja Media-palveluiden ja tallennustilaa avaimen perusteella arvojen päivittäminen ja arvoja. 
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Lisää koodi, joka käsittelee redundancy tarvittaessa streaming varten.



1. Lisää luokan tason kenttien ohjelma-luokka.
...
1. Korvaa yksi oletusarvon Main menetelmän määritys:
...
1. Menetelmä määritykset, joita kutsutaan pää on määritetty alla.
...
Voit nyt liikenteen hallinta, reitin pyyntöihin kaksi keskikohdan mukaan ja näin ollen automaattisesti kaikki katkokset kyseessä välillä.
