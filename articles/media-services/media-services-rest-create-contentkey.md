<properties 
    pageTitle="Luo ContentKeys muiden | Microsoft Azure" 
    description="Opettele luomaan sisällön näppäimet, joissa on suojattu käyttö resurssit." 
...
#<a name="create-contentkeys-with-rest"></a>Luo ContentKeys muiden kanssa
...
Media-palveluiden avulla voit luoda uusia ja pitää salattuja kalusto. **ContentKey** on suojattu käyttö **resurssi**s. 

Kun luot uuden resurssi (esimerkiksi ennen kuin voit [ladata tiedostoja](media-services-rest-upload-files.md)), voit määrittää seuraavat salausasetukset: **StorageEncrypted**, **CommonEncryptionProtected**tai **EnvelopeEncryptionProtected**. 

Kun pidät varat asiakkaiden, voit tehdä seuraavat kaksi salausta varten mainitun [dynaamisesti salattuja resurssien määrittäminen](media-services-rest-configure-asset-delivery-policy.md) : **DynamicEnvelopeEncryption** tai **DynamicCommonEncryption**.
...
1. Luo satunnaisesti 16-tavuinen AES avain (Yleinen ja kirjekuori salaus) tai 32-tavuinen AES avain (tallennustilan salaus). 
...
3.  Salaa sisällön key-tuotetunnuksen X.509-varmenne julkisella avaimella. 
    ...
    Media Services .NET SDK käyttävä RSA OAEP, kun teet salaus.  Näet esimerkki [funktiota EncryptSymmetricKeyData](https://github.com/Azure/azure-sdk-for-media-services/blob/dev/src/net/Client/Common/Common.FileEncryption/EncryptionUtils.cs).
4.  Luo tarkistussumman arvo (perustuu PlayReady AES tarkistussumma algoritmin) lasketaan avaimen tunnisteen ja sisällön avaimen avulla. Lisätietoja on kohdassa "PlayReady AES avain tarkistussumma algoritmin" PlayReady otsikon objektin asiakirja sijaitsee [seuraavassa](http://www.microsoft.com/playready/documents/).

    Seuraavassa esimerkissä .NET laskee tarkistussumma avaimen tunnisteen ja poista sisällön avain GUID-tunnus-osan avulla.
...
6. **ContentKey** -kohteen liittäminen **resurssi** -kohteen $links-toiminnon avulla.

Huomaa, että esimerkkejä, jotka Luo AES-näppäintä, avain salataan ja laskea tarkistussumma on jätetty pois tämän ohjeaiheen. Esimerkkejä, jotka osoittavat, miten voit käsitellä Media Services toimitetaan.
...
##<a name="create-the-contentkey"></a>Luo ContentKey 
...
Yksi arvoista, sinun on määritettävä milloin luominen sisältö avain on tyyppi. Valitse jokin seuraavista arvoista.
