<properties
   pageTitle="REST-pohjaisesta sovelluksesta elinkaari otoksen | Microsoft Azure"
   description="Microsoft Azure palvelun kangasta malli, jossa näkyy sovelluksen elinkaari-palvelun kangasta REST-käyttöliittymän avulla."
...
# <a name="rest-based-application-lifecycle-sample"></a>REST-pohjaisesta sovelluksesta elinkaari-Esimerkki

Tässä esimerkissä palvelun kangasta sovelluksen elinkaari REST API-kutsujen kautta. Lisätietoja palvelun kangasta sovelluksen elinkaari on artikkelissa [palvelun kangasta sovelluksen elinkaari](service-fabric-application-lifecycle.md).

Tässä esimerkissä suorittaa seuraavasti:

* Säännösten WordCount sovelluspaketin kuvassa **WordCount 1.0.0** näyte tallentaa.
* Näyttää luettelon sovelluksen tyypit, joka sisältää WordCount 1.0.0.
* Luo WordCount sovelluksen nimellä **kangasta: / WordCount**.
* Näyttää luettelon sovellukset, joka sisältää kangasta: / WordCount versio 1.0.0.
* Säännösten 1.1.0 **WordCountUpgrade** sovelluspaketin kuva kaupan WordCount näyte versio.
* Näyttää luettelon sovelluksen tyypit, joka sisältää WordCount 1.0.0 ja **WordCount 1.1.0**.
* Päivittää WordCount sovelluksen versioon 1.1.0.
* Näyttää luettelon sovelluksista, joka sisältää WordCount versio 1.1.0, mutta ei enää sisällä WordCount versio 1.0.0.
* Poistaa WordCount-sovellus.
* Näyttää luettelon sovellukset, joka sisältää enää kangasta: / WordCount.
* Unprovisions 1.1.0 WordCount otosten versio.
* Näyttää luettelon sovelluksen tyypit, joka sisältää WordCount 1.0.0, mutta ei enää sisällä WordCount 1.1.0.
* Unprovisions 1.0.0 WordCount otosten versio.
* Näyttää luettelon sovelluksen tyypit, joka sisältää enää WordCount.


## <a name="prerequisites"></a>Edellytykset

Tässä esimerkissä käytetään [WordCount-malli](http://aka.ms/servicefabricsamples) (löytyy **Käytön aloittaminen** -mallit). WordCount otosten rakennetaan ensin ja sitten kaksi sovelluksen paketit on kopioitava kuva-kaupasta.

|Kansion|Kuvaus|
|------|-----------|
|WordCount|WordCount-sovelluksen malli. **ApplicationManifest.xml** -tiedosto sisältää **ApplicationTypeVersion = "1.0.0"**.|
|WordCountUpgrade|WordCount-sovelluksen malli. ApplicationManifest.xml-tiedosto on muutettava **ApplicationTypeVersion = "1.1.0"** sallimaan sovelluksen päivitys suoritetaan.|

Luo sovelluksen-paketit ja kopioi ne kuva säilöön, seuraavasti:

1. Kopioi **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** **C:\Temp\WordCount**. Tämä luo WordCount sovelluspaketin.
2. Kopioi C:\Temp\WordCount **C:\Temp\WordCountUpgrade**. Tämä luo **WordCountUpgrade** sovelluspaketin.
3. Avaa tekstieditorissa **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** .
4. Muuta **ApplicationTypeVersion** -määritteen **ApplicationManifest** -elementin, **"1.1.0"**.  Sovelluksen versionumeron päivittyy.
5. Tallenna muutettu ApplicationManifest.xml-tiedosto.
6. Suorita järjestelmänvalvojana, Kopioi kuva-kaupan sovellukset seuraavaa PowerShell-komentosarjaa:
...
PowerShell-komentosarjaa päätyttyä tämä sovellus on valmis suorittamaan.

## <a name="example"></a>Esimerkki

Seuraavassa esimerkissä palvelun kangasta sovelluksen elinkaari.
